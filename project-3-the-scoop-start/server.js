...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    const articleCommentsIds = database.articles[savedComment.articleId].commentIds;
    userCommentsIds.splice(userCommentsIds.indexOf(id), 1);
    articleCommentsIds.splice(articleCommentsIds.indexOf(id), 1);
...
const requestHandler = (request, response) => {
  ...
  // Establece como tienen que ser las request
  if (method === 'OPTIONS') {
    var headers = {};
