...
const orderedMedals = (country, field, sortAscending) => {
  ...
  if (field && sortAscending !== undefined) {
    let sortOrder = sortAscending ? 'asc' : 'desc';

    sortQuery = `ORDER BY ${field} ${sortOrder}`;
  }
...
const orderedSports = (country, field, sortAscending) => {
  ...
  return `WITH total AS (SELECT COUNT(*) AS countMedals
  FROM GoldMedal
  WHERE country = '${country}')

  SELECT sport, COUNT(*) AS count, ROUND(COUNT(*) * 100.0 / countMedals, 0) as percent
  FROM GoldMedal
  CROSS JOIN total
  WHERE country = '${country}'
  GROUP BY sport
